...
@pytest.mark.asyncio
async def test_cors_credentialed_return_specific_origin(app, client):
...
@pytest.mark.asyncio
async def test_cors_vary_headers_origin_default(app, client):
...
@pytest.mark.asyncio
async def test_cors_vary_headers_set(app, client):
    app.add_middleware(CorsMiddleware, allow_origins=["http://testserver.com"])
